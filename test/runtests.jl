...
@testset "REVOLT Classic HSDM277 command geometry" begin
...
@testset "REVOLT Classic graph profiles" begin
    ...
    @test basename(rtc_reference_graph_path()) ==
    ...
    for profile in supported_profiles()
        ...
        @test graph_name(graph) === Symbol("revolt_classic_hil_", profile)
        @test size(hil_frame_buffer(boundary)) == (352, 352)
        ...
        hil_command_buffer(boundary)[143] = 5.0f-8
...
@testset "REVOLT Classic calibration preparation" begin
    mask = valid_subapertures()
    @test size(mask) == (16, 16)
    @test count(mask) == 188

    calibration = prepare_calibration_system()
    @test graph_name(calibration.graph) === :revolt_classic_hil_calibration
    @test size(hil_frame_buffer(calibration.boundary)) == (352, 352)
...
@testset "REVOLT Classic deterministic replay" begin
...
@testset "REVOLT Classic science diagnostics" begin
    diagnostics = prepare_science_diagnostics()
    zero_opd = zeros(Float32, 240, 240)
    update_science_diagnostics!(diagnostics, zero_opd, zero_opd)
    @test size(science_pupil_support(diagnostics)) == (240, 240)
    @test size(open_loop_psf(diagnostics)) == (480, 480)
    @test size(closed_loop_psf(diagnostics)) == (480, 480)
...
if get(ENV, "REVOLT_CLASSIC_PYRTC_TESTS", "0") == "1"
    include(joinpath(@__DIR__, "pyrtc", "test_revolt_classic_hil.jl"))
end
